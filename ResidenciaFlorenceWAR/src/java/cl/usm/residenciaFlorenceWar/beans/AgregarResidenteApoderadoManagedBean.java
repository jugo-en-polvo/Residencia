...
public class AgregarResidenteApoderadoManagedBean implements Serializable {
    ...
    public void agregar(ActionEvent e) {
        ...
        if (residenteDAO.compruebaExistencia(residenteBean.getRutResidente())) {
            ...
            if (VerificarEmail(apoderadoBean.getEmailApoderado())) {
                if (ValidadorRut.validarRut(apoderadoBean.getRutApoderado()) && ValidadorRut.validarRut(residenteBean.getRutResidente())) {
                    ...
                    if (!ValidadorRut.validarRut(apoderadoBean.getRutApoderado())) {
                        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error: Rut de Apoderado inválido.", ""));
                    } else if (!ValidadorRut.validarRut(residenteBean.getRutResidente())) {
                        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error: Rut de Residente inválido.", ""));
                    } else {
                        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error: Rut de Apoderado y Residente inválido.", ""));
                    }
                }
            }
